"""
This file is copied from [clustered-federated-learning/models.py]
        https://github.com/felisat/clustered-federated-learning/blob/master/helper.py
"""
...
def display_train_stats(cfl_stats, eps_1, eps_2, communication_rounds):
    ...
    plt.subplot(1, 2, 1)
    acc_mean = np.mean(cfl_stats.acc_clusters, axis=1)
    acc_std = np.std(cfl_stats.acc_clusters, axis=1)
    ...
    plt.text(x=communication_rounds, y=1, ha="right", va="top",
             s=f"Clusters: {cfl_stats.clusters[-1]}")
