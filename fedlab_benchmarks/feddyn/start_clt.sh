...
WorldSize=$(($ClientRankNum + 1))
# balance iid cifar10 for 100 clients, check config.py for other setting
python data_partition.py --out-dir ./Output/debug --partition iid --balance True --dataset cifar10 --num-clients ${ClientNum} --seed 0
echo -e "Data partition DONE.\n\n"
sleep 4s
...
for ((i = 1; i <= ${ClientRankNum}; i++)); do
  {
    echo "client ${i} started"
    python client_starter.py --world_size ${WorldSize} --rank ${i} --client-num-per-rank ${ClientNumPerRank} --alg FedDyn --out-dir ./Output/debug &
    sleep 2s
  }
done
